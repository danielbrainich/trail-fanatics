...
function PostLikeButton({ postId }) {
        ...
        if (user) {
            const fetchLikeStatus = async () => {
                const response = await fetch(
                    `${baseUrl}/content/posts/${postId}/check-like/`,
                    {
                        credentials: "include",
                    }
                );
    ...
    const toggleLike = async () => {
        if (!liked) {
            try {
                const response = await fetch(
                    `${baseUrl}/content/posts/${postId}/post-likes/`,
                    {
                        method: "POST",
                        headers: {
            ...
            try {
                if (likeId) {
                    const response = await fetch(
                        `${baseUrl}/content/posts/${postId}/post-likes/${likeId}/`,
                        {
                            method: "DELETE",
                            headers: {
                                "Content-Type": "application/json",
            ...
            {user ? (
                ...
                <>
                    <button
                        ...
                        data-bs-target="#noLikePostModal"
                        tabIndex="0"
                        style={{ border: "none", background: "transparent" }}
                    >
                        <FontAwesomeIcon
                    ...
                    <div
                        ...
                        tabIndex="-1"
                        aria-labelledby="noLikePostModalLabel"
                        aria-hidden="true"
                    >
